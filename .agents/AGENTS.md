...
2. **Local AI Chunking**: The project uses a local Ollama LLM (auto-detects `llama3` or `llama3.2`) to format extracted texts into strict RAG structures (`Core Summary`, `Key Concepts & Definitions`, `Important Details / Application`, `Original Context & Quotes` + up to 3 dynamic context-specific headers). If you modify the chunking logic or prompt, ensure you maintain anti-redundancy rules, enforce Markdown table formatting for data, and strictly demand a JSON output containing `filename_slug`, `tags`, and `rag_content`.
