...
## Core Directives
1. **RAG-Ready Output**: The ultimate goal of this project is to feed the generated markdown into a Retrieval-Augmented Generation (RAG) system specialized in Stock Trading. Never output monolithic files. Always split content intelligently using `utils/chunking.py`.
2. **Local AI Chunking**: The project uses a local Ollama LLM (`llama3`) to format extracted texts into specific Trading structures (`Key Concept`, `Trading Rules`, `Scanner Criteria`). If you modify the chunking logic, ensure the prompt strictly demands a JSON output containing `filename_slug`, `tags`, and `rag_content`.
